...
from sklearn.metrics import accuracy_score, recall_score, f1_score, log_loss, classification_report, confusion_matrix
...
class_counts = df['estado_vuelo'].value_counts()
print("✅ Estado de clases disponibles antes del split:")
print(class_counts)

# Filtrar clases con al menos 5 ejemplos
valid_classes = class_counts[class_counts >= 5].index
df = df[df['estado_vuelo'].isin(valid_classes)]
...
# Validación final
if y.nunique() < 2:
    raise ValueError("❌ No hay suficientes clases distintas para entrenar. Revisa el balance de datos.")
...
print(classification_report(y_test, y_pred, target_names=le_estado.inverse_transform(sorted(y.unique()))))

# Matriz de confusión
conf_mat = confusion_matrix(y_test, y_pred)
plt.figure(figsize=(6, 5))
sns.heatmap(conf_mat, annot=True, fmt='d', cmap='Blues',
            xticklabels=le_estado.inverse_transform(sorted(y.unique())),
            yticklabels=le_estado.inverse_transform(sorted(y.unique())))
...
print(f"{'Modelo':<22} | {'Precisión':<10} | {'Sensibilidad':<12} | {'F1-score':<10} | {'LogLoss':<8}")
print("-" * 75)
